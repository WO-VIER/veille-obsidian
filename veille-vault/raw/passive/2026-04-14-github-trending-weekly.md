---
kind: passive-capture
source_name: "GitHub Trending Weekly"
source_type: secondaire
category: github-trending
tags: [github, trending, weekly, veille]
title: "GitHub Trending Weekly - 2026-04-14"
url: "https://github.com/trending?since=weekly"
published_at: "2026-04-14T09:11:53.483Z"
collected_at: "2026-04-14T09:11:53.483Z"
confidence: medium
workflow: github-trending-weekly
status: a-traiter
---

# GitHub Trending Weekly - 2026-04-14

## Metadonnees
- Source : GitHub Trending
- Type : secondaire
- URL : https://github.com/trending?since=weekly
- Portee : top 10 repositories visibles au moment de la collecte

## Repositories
- 1. forrestchang / andrej-karpathy-skills
  - URL : https://github.com/forrestchang/andrej-karpathy-skills
  - Langage : Langage non précisé
  - Description : A single CLAUDE.md file to improve Claude Code behavior, derived from Andrej
Karpathy's observations on LLM coding pitfalls.
- 2. NousResearch / hermes-agent
  - URL : https://github.com/NousResearch/hermes-agent
  - Langage : Python
  - Description : The agent that grows with you
- 3. multica-ai / multica
  - URL : https://github.com/multica-ai/multica
  - Langage : TypeScript
  - Description : The open-source managed agents platform. Turn coding agents into real teammates
— assign tasks, track progress, compound skills.
- 4. HKUDS / DeepTutor
  - URL : https://github.com/HKUDS/DeepTutor
  - Langage : Python
  - Description : "DeepTutor: Agent-Native Personalized Learning Assistant"
- 5. coleam00 / Archon
  - URL : https://github.com/coleam00/Archon
  - Langage : TypeScript
  - Description : The first open-source harness builder for AI coding. Make AI coding
deterministic and repeatable.
- 6. microsoft / markitdown
  - URL : https://github.com/microsoft/markitdown
  - Langage : Python
  - Description : Python tool for converting files and office documents to Markdown.
- 7. google-ai-edge / gallery
  - URL : https://github.com/google-ai-edge/gallery
  - Langage : Kotlin
  - Description : A gallery that showcases on-device ML/GenAI use cases and allows people to try
and use models locally.
- 8. google-ai-edge / LiteRT-LM
  - URL : https://github.com/google-ai-edge/LiteRT-LM
  - Langage : C++
  - Description : Pas de description.
- 9. NVIDIA / personaplex
  - URL : https://github.com/NVIDIA/personaplex
  - Langage : Python
  - Description : PersonaPlex code.
- 10. TheCraigHewitt / seomachine
  - URL : https://github.com/TheCraigHewitt/seomachine
  - Langage : Python
  - Description : A specialized Claude Code workspace for creating long-form, SEO-optimized blog
content for any business. This system helps you research, write, analyze, and
optimize content that ranks well and serves your target audience.

## Action possible
- [ ] Decider si un ou plusieurs repositories meritent une vraie note dans `wiki/notes/`
